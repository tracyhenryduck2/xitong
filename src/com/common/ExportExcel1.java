...
public class ExportExcel1 {
	 ...
	 */
	public ExportExcel1(String fileName, HttpServletResponse	response){
		try {
	...
	public void createHead(){
		...
		if(getHeadArray()!=null){
			for(int i=0;i<getHeadArray().length;i++){ 
				  label=new Label(i,0,getHeadArray()[i]);//参数依次代表列数、行数、内容 
				 try{
					 if(widthArray!=null){
						 //System.out.println(widthArray[i]);
	 ...
	 * @param maps
	 */
	public void addData(List<Map<String,Object>> maps,String[] array){
		...
		for(Map<String,Object> map :maps){
			...
			for(int i=0; i<headArray.length; i++){
				...
				if(dataTypeArray[i].equals(TYPE_NORMAL)){
					label = new Label(i,currentRow,str);
				}else if(dataTypeArray[i].equals(TYPE_DATE)){
					label = new Label(i,currentRow,convertTimeNumToDate(str,null));
				}else if(dataTypeArray[i].equals(TYPE_IP)){
					label = new Label(i,currentRow,convertIPNumToDot(str));
				}
	...
	private void createHead(String[] array) {
		Label label = null;
		if(array!=null){
			for(int i=0;i<array.length;i++){ 
				  label=new Label(i,0,array[i]);//参数依次代表列数、行数、内容 
				 try{
		...
		if(getHeadArray()!=null){
			for(int i=0;i<getHeadArray().length;i++){ 
				  label=new Label(i,1,getHeadArray()[i]);//参数依次代表列数、行数、内容 
				 try{
					 if(widthArray!=null){
						 //System.out.println(widthArray[i]);
						 worksheet.setColumnView(i,Integer.parseInt(widthArray[i]));
