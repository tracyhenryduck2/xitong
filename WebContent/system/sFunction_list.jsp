...
<html>    
	...
	<script type="text/javascript">    
		...
		function search() {  
			Dialog.open({Title:"查询", Width:650, Height:220, URL:"<%=path%>/system/SFunction!search.action",OnLoad:function(){
			     ...
			     this.innerDoc.getElementById("function_group_id").value = $("#function_group_id").val();
			     this.innerDoc.getElementById("sort").value = $("#sort").val();
			     this.innerDoc.getElementById("authority").value = $("#authority").val();
           }
			});    
		}      
  ...
  <GF:BodyCaption label="功能" ico="images/ico/user.gif"> 
    ...
    <form action="<%=path%>/system/SFunction!list.action" name="form1" id="form1" method="get">  
			...
			<input type="hidden" name="sFunctionBean.functionGroupId" id="functionGroupId" value="${sFunctionBean.functionGroupId}"/> 
			<input type="hidden" name="sFunctionBean.sort" id="sort" value="${sFunctionBean.sort}"/>
						<input type="hidden" name="sFunctionBean.authority" id="authority" value="${sFunctionBean.authority}"/>  
			...
			<table id="table1" width="100%" height="100%"  border="0" cellspacing="0" cellpadding="0"> 
			   	 <thead>
			   		<tr>  
			   			...
			   			<th sortname="url" width="10%">地址</th>	   
			   			<th sortname="function_group_id" width="10%">模块</th>	   
			   			<th sortname="sort" width="10%">排序</th>
			   			<th sortname="authority" width="10%">权限</th>		   
			   		</tr> 
			   	</thead>
			   	<tbody> 
