...
<html>    
	...
	<script type="text/javascript">    
		...
		function del(){      
			...
			Dialog.confirm("确定删除吗", function(){     
				...
				form1.action = "<%=path%>/memberManage/FeeLog!delFeeLogs.action";
				form1.target = "fram";
				form1.submit();  
		...
		function search() {  
			Dialog.open({Title:"查询", Width:650, Height:220, URL:"<%=path%>/memberManage/FeeLog!search.action",OnLoad:function(){
  ...
  <body>  
  <GF:BodyCaption label="消费记录" ico="images/ico/user.gif"> 
  	<GF:ToolBar id="123">     
    ...
    <form action="<%=path%>/memberManage/FeeLog!list.action" name="form1" id="form1" method="get">  
   
			<input type="hidden" name="feeLogBean.feeMoney" id="feeMoney" value="${feeLogBean.feeMoney}"/> 
			<input type="hidden" name="feeLogBean.feeTime" id="feeTime" value="${feeLogBean.feeTime}"/> 
			<input type="hidden" name="feeLogBean.initStorage" id="initStorage" value="${feeLogBean.initStorage}"/> 
			...
			<table id="table1" width="100%" height="100%"  border="0" cellspacing="0" cellpadding="0"> 
			   	 <thead>
			   		<tr>  
			   			...
			   			<th sortname="fee_money" width="10%">消费金额</th>	   
			   			<th sortname="charge_time" width="10%">消费时间</th>	   
			   	...
			   	<tbody> 
			   		<s:iterator value="#request.list" id="map"> 
		   			 <tr> 
		   			 	 ...
		   			 	 <td>${map.fee_money}</td>    
		   			 	 <td><GF:ConvertTime value="${map.fee_time}" format="yyyy-MM-dd HH:mm:ss"/></td>    
